<template>
  <div>
    ...
    <el-row type="flex" justify="center">
      <el-col :span=8 v-for="item in barInfo" style="margin-top: 50px">
        <div class="container">
          <div class="row">
            <div class="col-md-3 col-sm-6">
              <div class="product-grid">
                <div class="product-image">
                  <a href="#" class="image">
                    <img style="width: 200px" class="pic-1" :src="loadImg(item.barHead)">
                  </a>
                </div>
                <div class="product-content">
                  <h3 class="title"><a href="#">{{ item.barName }}</a></h3>
                  <div class="price">{{ item.barIntroduction }}</div>
                  <router-link class="add-to-cart" :to="{path:'/index/barView?barName='+item.barName}">
                    <i class="fas fa-shopping-cart"></i> <span>进来瞧瞧</span>
                  </router-link>
                </div>
              </div>
            </div>
...
export default {
  ...
  created() {
    axios.get("/index/bar/getBar").then((res)=>{
      // console.log(res);
