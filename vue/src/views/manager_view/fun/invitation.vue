...
export default {
  ...
  methods: {
    ...
    handleDelete(index, row) {
        this.$confirm('此操作将删除该帖子, 是否继续?', '提示', {
          ...
          axios.post("/manage/invitation/deleteInvitation", row).then(() => {
            //请求一个页的数据,好让数据刷新
            axios.post("/manage/invitation/getInvitaionBySearchPage/"+this.currentPage, {searchWord: this.searchWord, value: this.value}).then((res) => {
              // console.log(res);
    ...
    search() {
      ...
      axios.post("/manage/invitation/getInvitaionBySearchPage/1", {searchWord: this.searchWord, value: this.value}).then((res) => {
        // console.log(res);
        this.$data.tableData = res.data.data.records;
        this.total = res.data.data.total;
      });
  ...
  created(searchWord, value) {
    ...
    axios.post("/manage/invitation/getInvitaionBySearchPage/1", {searchWord: searchWord, value: value}).then((res) => {
      // console.log(res);
      this.$data.tableData = res.data.data.records;
      this.total = res.data.data.total;
    });
